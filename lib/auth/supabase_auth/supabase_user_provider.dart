...
class RelaxSupabaseUser extends BaseAuthUser {
  RelaxSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> relaxSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = RelaxSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
